<template>
    <div class="tree-page">
        ...
        <div class="tree-toolbar" v-if="toolbarVisible">
        ...
        <NodeDetailModal v-if="yhlx !== 'preview'" ref="nodeDetailModal" :activeNode="nodeDetailActiveNode"
            :sfzyxxms="sfzyxxms" :kcfmUrl="kcfmUrl" :xxjd="xxjd" />
...
<script>
import kctpJsMindComponent from './jsmind/index.vue'
import NodeDetailModal from '../NodeDetailModal.vue'
import { mapState } from 'vuex'

import kcjg from '@/pages/ryxt/kcjg/index.vue'
export default {
    name: 'TreeGraph',
    components: {
        kctpJsMindComponent,
        ...
        return {
            ...
            jsMindOptions: {
                ...
                view: {
                    ...
                    expander_style: 'char',
                    hmargin: (window.innerWidth / 2) - 50// 自动计算屏幕宽度的一半
                },
                layout: {
                    hspace: 30,
                    vspace: 50,
    ...
    computed: {
        ...
        toolbarVisible() {
            return this.yhlx !== 'preview' && (this.yhlx !== 'student' || this.openInfo)
        }
    },
    ...
    methods: {
        ...
        /**
         * 数据挂载后执行导图初始化：连线补丁、事件绑定、默认收起、缩放和居中
         * @returns {void}
         */
        initializeMindView(retryCount = 0) {
        ...
        /**
         * mind实例未就绪时进行短轮询，避免首次渲染时序导致事件未绑定
        ...
        handleSearchLocate() {
            ...
            const targetNode = this.pickNextMatchedNode(keyword)
            if (!targetNode) {
        ...
        pickNextMatchedNode(keyword) {
            ...
            const targetId = this.searchMatchedNodeIds[this.searchCursor]
            return this.getMindNodeById(mindInstance, targetId)
        },

        /**
         * 展开目标节点祖先链并高亮当前节点，同时滚动到可视区域中心
        ...
        /**
         * 展开目标节点的祖先链，保证目标节点可见
         * @param {Object} mindInstance jsmind实例
         * @param {Object} node 目标节点
         * @returns {void}
         */
        ...
        scrollNodeIntoView(node) {
            const element = node && node._data && node._data.view ? node._data.view.element : null
        ...
        fitMindToContainerWidth() {
            ...
            const view = mindInstance.view
            const setZoom = typeof view.set_zoom === 'function'
                ? view.set_zoom.bind(view)
                : (typeof view.setZoom === 'function' ? view.setZoom.bind(view) : null)
            if (!setZoom) return false

            const panel = view.e_panel
            const nodes = panel ? panel.querySelector('jmnodes') : null
            if (!panel || !nodes) return false

            const panelWidth = panel.clientWidth
            const panelHeight = panel.clientHeight
            const sizeWidth = view.size && view.size.w ? view.size.w : (nodes.scrollWidth || nodes.clientWidth)
            const sizeHeight = view.size && view.size.h ? view.size.h : panelHeight
            if (!sizeWidth || !panelWidth || !sizeHeight || !panelHeight) return false

            const zoomConfig = this.jsMindOptions && this.jsMindOptions.view
                ? this.jsMindOptions.view.zoom
                : null
            const minZoom = zoomConfig && Number.isFinite(zoomConfig.min) ? zoomConfig.min : 0.5
            const maxZoom = zoomConfig && Number.isFinite(zoomConfig.max) ? zoomConfig.max : 2.1

            // jsMind 内部会拒绝“宽高同时小于面板”的缩放，这里取宽高拟合中的较大值并加微量余量
            const widthFitZoom = panelWidth / sizeWidth
            const heightFitZoom = panelHeight / sizeHeight
            const rawZoom = Math.max(widthFitZoom, heightFitZoom) + 0.01
            const targetZoom = Math.max(minZoom, Math.min(maxZoom, Number(rawZoom.toFixed(2))))
        ...
        handleMindNodeSelect(payload) {
            const selectedNodeId = payload && payload.nodeId ? payload.nodeId : ''
            if (!selectedNodeId) return
            this.deferNodeSelectAction(selectedNodeId)
        },

        /**
         * 业务侧重写连线绘制，实现圆角肘形连线与节点颜色联动
         * @returns {boolean} 是否成功挂载绘制补丁
         */
        applyCustomLineRenderer(mindInstanceParam) {
            const mindInstance = mindInstanceParam || this.getMindInstance()
            ...
            const originalShowLines = typeof view.show_lines === 'function' ? view.show_lines.bind(view) : null
            const componentVm = this

            // 仅在当前业务页面覆写连线绘制，保持通用组件简洁
            view.show_lines = function () {
                ...
                graph.clear()
                const ctx = graph.canvas_ctx
                const nodes = mindInstance.mind && mindInstance.mind.nodes ? mindInstance.mind.nodes : {}
                const offset = typeof view.get_view_offset === 'function' ? view.get_view_offset() : { x: 0, y: 0 }
                const lineWidth = componentVm.jsMindOptions && componentVm.jsMindOptions.view
                    ? componentVm.jsMindOptions.view.line_width
                    : 2
                const defaultColor = componentVm.jsMindOptions && componentVm.jsMindOptions.view
                ...
                Object.keys(nodes).forEach((nodeId) => {
                    ...
                    try {
                        const pin = mindInstance.layout.get_node_point_in(node)
                        const pout = mindInstance.layout.get_node_point_out(node.parent)
                        const lineColor = node.data && node.data['line-color'] ? node.data['line-color'] : defaultColor
                        const rootLineExtraOffset = node.parent && node.parent.isroot ? -20 : 0// -20向左偏移
        ...
        /**
         * 延迟节点选中处理到鼠标抬起后，确保“点击完成后”再触发业务逻辑。
        ...
        deferNodeSelectAction(nodeId) {
            ...
            this.pendingSelectMouseupHandler = () => {
                const mindInstance = this.getMindInstance()
                const selectedNode = this.getMindNodeById(mindInstance, this.pendingSelectNodeId)
        ...
        /**
         * 根据节点数据恢复选中态颜色
         * @param {Object|null} node jsmind节点对象
         * @returns {void}
         */
        applySelectedNodeStyle(node) {
            if (!node || !node.data) return

            const titleElement = this.getNodeTitleElement(node)
            if (!titleElement) return

            const backgroundColor = node.data['background-color']
            const foregroundColor = node.data['foreground-color']
        ...
        getLineConfigValue(key, fallback) {
            const value = Number(this.customLineConfig && this.customLineConfig[key])
         ...
         */
        getMindInstance() {
            const component = this.$refs.treeGraphMind
        ...
        /**
         * 通过节点ID获取节点对象
         * @param {Object|null} mindInstance jsmind实例
         * @param {string} nodeId 节点ID
         * @returns {Object|null}
         */
         ...
         * @param {number} x1 父节点出线点x
         * @param {number} y1 父节点出线点y
         * @param {number} x2 子节点入线点x
         * @param {number} y2 子节点入线点y
         * @param {string} color 连线颜色
         * @param {number} width 连线宽度
         * @param {number} extraOffset 额外肘点偏移（用于按需加长末端横线）
         * @returns {void}
         */
        drawRoundedElbowLine(ctx, x1, y1, x2, y2, color, width, extraOffset = 0) {
            ...
            const elbowOffset = this.getLineConfigValue('elbowOffset', 30)
            const maxCornerRadius = this.getLineConfigValue('maxCornerRadius', 2.5)
            const elbowX = x2 - dirX * (elbowOffset + Number(extraOffset || 0)) // 肘点偏移，用于末端横线长度
            const tabRadius = Math.min(maxCornerRadius, Math.abs(y2 - y1) / 2) // 底部转角圆角

            ctx.moveTo(x1, y1)
            ctx.lineTo(elbowX, y1)
            ctx.lineTo(elbowX, y2 - dirY * tabRadius)
            ctx.quadraticCurveTo(elbowX, y2, elbowX + dirX * tabRadius, y2)
            ctx.lineTo(x2, y2) // 末端横线
            ctx.stroke()
        },

        /**
         * 将后端树结构转换为 jsmind 的 node_tree 格式
         * @param {Object} treeData 根节点数据
         * @returns {Object}
         */
        convertToJsMindFormat(treeData) {
             ...
             * @param {number} rootBranchIndex 一级分支索引
             * @param {number} childIndex 同级索引
             * @returns {string}
             */
            const createSafeId = (node, depth, rootBranchIndex, childIndex) => {
                const candidates = [
                    node && node.id,
                    node && node.workId,
                    node && node.lmid,
                ...
                let baseId = candidates.find(item => item !== undefined && item !== null && String(item).trim() !== '')
                if (baseId === undefined || baseId === null || String(baseId).trim() === '') {
                    const topic = (node && node.lmmc ? String(node.lmmc).trim() : '') || 'node'
                    baseId = `${topic}-${depth}-${rootBranchIndex}-${childIndex}`
                }

                let safeId = String(baseId)
             ...
             * @returns {Object}
             */
            const convertNode = (node, depth = 0, rootBranchIndex = 0) => {
                const isLeaf = !node.children || node.children.length === 0
                const palette = this.nodecolor[rootBranchIndex % this.nodecolor.length]
                const colorType = this.getNodeColorType({ depth, isLeaf, lmlx: node.lmlx })
                const nodeColor = this.getNodeColorByType(palette, colorType)
                const childIndex = Number.isInteger(node.__childIndex) ? node.__childIndex : 0
                const currentId = createSafeId(node, depth, rootBranchIndex, childIndex)
                const nodedata = { ...node }
                if (nodedata.lmlx != 2) {
                ...
                const children = (node.children || [])
                    .filter(child => {
                        if (this.yhlx !== 'student') {
                            if (Number(child.lmlx) === 3) {
                                const nrlx = Number(child.nrlx)
                                return nrlx === 7 || nrlx === 8 || nrlx === 9
                            }
                        }
                        return true
                    })
                    .map((child, index) => {
                        const nextBranchIndex = depth === 0 ? index : rootBranchIndex
                        return convertNode({ ...child, __childIndex: index }, depth + 1, nextBranchIndex)
                    })
                const baseNode = {
                    id: currentId,
                    topic: this.buildNodeTopic(node, {
                        isRoot: depth === 0,
                        nodeColor,
         ...
         * @param {number} params.depth 节点层级
         * @param {boolean} params.isLeaf 是否叶子节点
         * @param {number|string} params.lmlx 节点类型
         * @returns {'one'|'two'|'three'}
         */
        getNodeColorType({ depth, isLeaf, lmlx }) {
            if (depth === 1) return 'one'
            if (isLeaf && Number(lmlx) === 3) return 'three'
            return 'two'
        },

        /**
         * 根据颜色层级从调色板中取色
         * @param {Object|null} palette 调色板
         * @param {'one'|'two'|'three'} colorType 颜色类型
         * @returns {string}
         */
         ...
         * @param {boolean} options.isRoot 是否根节点
         * @param {string} options.nodeColor 节点背景色
         * @param {string} options.foregroundColor 节点字体色
         * @returns {string}
         */
        buildNodeTopic(node, options = {}) {
            const title = node && node.lmmc ? String(node.lmmc) : '未命名节点'
            const escapedTitle = this.escapeHtml(title)
            const isRoot = Boolean(options.isRoot)
            const nodeColor = options.nodeColor || '#97edf9'
            ...
            if (node && (node.lmlx == 1 || node.lmlx == 0)) {
                const percentage = `<div class="tg-node-title-border">
            ...
            if (this.isLeaf(node)) {
                let zsdTags = `<span class="tg-zsd-tag tg-node-tag">知识点</span>`
                const tags = []
                if (Number(node && node.zd) === 1) tags.push('重点')
                if (Number(node && node.nd) === 1) tags.push('难点')
                if (Number(node && node.kd) === 1) tags.push('考点')
                const tagHtml = tags
                    .map(label => `<span class="tg-node-tag">${label}</span>`)
                    .join('');
                const zsdzt = ` 
                <div class="tg-node-zsdzt">
                ...
                return `<div class="tg-node-content tg-node-content--knowledge">
                    ...
                    ${node.lmlx == 2 ? `<div class="tg-node-tags">${zsdTags}${tagHtml}</div>` : `<div class="tg-node-tags">${aa}</div>`}
                    </div>`;
            } else {
                return ``;
            }
        ...
        isLeaf(data) {
            return (data.lmlx == 3 && (data.nrlx == 7 || data.nrlx == 8 || data.nrlx == 9 || data.nrlx == 10)) || data.lmlx == 2;
        ...
        handleNodeClick(node) {
            ...
            if (Number(nodeData.lmlx) !== 2) {
                if (!(this.yhlx === 'student' && Number(nodeData.lmlx) === 3 && [7, 8, 9, 10].includes(Number(nodeData.nrlx)))) return
            }

            const zsdId = nodeData.zsdid
